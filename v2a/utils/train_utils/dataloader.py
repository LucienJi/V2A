...
from .dataset import EncoderSequenceDataset,PolicyDataset
...
def policy_dataset_factory(config, obs_keys, dataset_path=None):
    if dataset_path is None:
        dataset_path = config.train.data
    ...
    ds_kwargs = dict(
        ...
        hdf5_use_swmr=config.train.hdf5_use_swmr,
        hdf5_normalize_obs=config.train.hdf5_normalize_obs,
        with_encoder = config.train.with_encoder
    )
    dataset = PolicyDataset(**ds_kwargs)

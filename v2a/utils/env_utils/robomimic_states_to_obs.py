...
from .env_robosuite_v2 import EnvRobosuite3D
...
def robomimic_states_to_obs(args):
    # create environment to use for data processing
    env_meta = FileUtils.get_env_metadata_from_dataset(dataset_path=args.dataset)
    env_kwargs = env_meta["env_kwargs"]
    env_name=env_meta['env_name'],
    if type(env_name) == tuple:
        env_name = env_name[0]
    ...
    env = EnvRobosuite3D.create_for_data_processing(
            env_name=env_name,
    ...
    print(OmegaConf.to_yaml(env.serialize()))    
    print("")    
    ...
    other_grp = f_out.create_group("other")

    ## save the intrinsic camera parameters and extrinsic camera parameters in the data_grp
    if args.camera_names is not None:
        ## save the intrinsic camera parameters and extrinsic camera parameters 
        intrinsic_grp = other_grp.create_group("camera_intrinsic")
        extrinsic_grp = other_grp.create_group("camera_extrinsic")
        for name in args.camera_names:
            intrinsic_grp.create_dataset(name,data = np.array(get_camera_intrinsic_matrix(sim = env.env.sim, camera_name=name, camera_height=args.camera_height, camera_width=args.camera_width)))
            extrinsic_grp.create_dataset(name,data = np.array(get_camera_extrinsic_matrix(camera_name=name, sim=env.env.sim)))  
